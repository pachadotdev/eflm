...
test_that("Drop: flgm (gaussian) == glm with no test", {
  ...
  m2 <- bglm(mpg ~ wt + am, data = mtcars, family = gaussian())
  drop_m2 <- drop1(m2)
...
test_that("Drop: flgm (inverse.gaussian) == glm with no test", {
  ...
  m2 <- bglm(mpg ~ wt + am, data = mtcars, family = inverse.gaussian())
  drop_m2 <- drop1(m2)
...
test_that("Drop: flgm (poisson) == glm with no test", {
  ...
  m2 <- bglm(mpg ~ wt + am, data = mtcars, family = quasipoisson())
  drop_m2 <- drop1(m2)
...
test_that("Drop: fails without model matrix", {
  m1 <- bglm(mpg ~ wt + am, data = mtcars, family = gaussian(), model = FALSE)
