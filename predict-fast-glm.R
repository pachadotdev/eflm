predict.fast_glm <- function(object, newdata, type = c("link", "response"),
                             na.action = na.pass, ...) {
  type <- match.arg(type)
  if (missing(newdata) & is.null(object$linear.predictors)) {
    warning("Fitted values were not returned from the fast_glm object:
  ...
  if (missing(newdata)) {
    ...
    pred <- predict.fast_lm(object, newdata,
      type = "response",
      na.action = na.action
    )
