...
#' @export
#' @keywords internal
print.summary.bglm <- function(x, digits = max(3, getOption("digits") - 3), ...) {
  if (!is.null(x$call)) {
    cat("\nCall:\n")
    cat(deparse(x$call))
    cat("\n\n")
...
#' @export
#' @keywords internal
print.bglm <- function(x, digits = max(3, getOption("digits") - 3), ...) {
...
#' @keywords internal
print.logLik.bglm <- function(x, digits = getOption("digits"), ...) {
