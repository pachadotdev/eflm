...
#' @export
#' @keywords internal
update.elm <- function(object, formula, data, subset = NULL, add = TRUE,
                       evaluate = TRUE,
  ...
  if ((!missing(data)) & (add)) {
    mod <- update_with_more_data(object, data,
      subset = subset,
    ...
    mod <- if (missing(data)) {
      update.default(object, formula,
        evaluate = evaluate, subset = subset,
        offset = NULL, weights = NULL, ...
      )
    } else {
      update.default(object, formula,
        data = data, evaluate = evaluate,
